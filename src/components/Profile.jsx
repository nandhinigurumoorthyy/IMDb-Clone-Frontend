...
const Profile = () => {
  ...
  const fetchMovies = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`https://imdb-clone-backend-o1bt.onrender.com/api/user-movies/${userid}`);
      setMovies(data);
    } catch (error) {
  ...
  const handleEditSubmit = async (e, movieId) => {
    e.preventDefault();
    try {
      await axios.put(`https://imdb-clone-backend-o1bt.onrender.com/api/movies/${movieId}`, editedMovie);
      await fetchMovies();
  ...
  const handleDelete = async (movieId) => {
    if (window.confirm("Are you sure you want to delete this movie?")) {
      try {
        await axios.delete(`https://imdb-clone-backend-o1bt.onrender.com/api/movies/${movieId}`);
        await fetchMovies();
